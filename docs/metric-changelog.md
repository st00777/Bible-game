...
| 2026-09-13 | `read_chapter`（閱讀勳章／閱讀率，回頭更新 2026-08-23 條的判斷） | 機制本身未變（08-23 上線的「我已經讀過」自陳按鈕），但 `source='already'` 佔比連三期上升（章節口徑 8%→16%→21%、人頭口徑 12%→33%→29%），09-06~09-12 已滿週仍在 20%+，超過 9/9 快照設定的觀察門檻，正式列入口徑漂移追蹤 | 09-06~09-12 起「閱讀率」不可再當單一乾淨指標讀，需拆「外連 bible_com」與「自述 already」兩條線分開判讀；08-23 條原判斷「非口徑造成、仍可比」僅適用於當時 2% 的低佔比階段，不再適用於本期之後的數字 |
| 2026-09-19 | `read_chapter`（閱讀勳章／閱讀率，延續 09-13 條追蹤） | 機制仍未變；`source='already'` 章節口徑本期續升（21%→27%），但人頭口徑本期持平（29%→29%，同 4/14 人）── 漂移方向未逆轉，惟擴散速度趨緩 | 09-13~09-19 起判讀持續沿用 09-13 條的「拆兩條線讀」原則，兩口徑本期出現分化（章節續升／人頭持平），下期需同時追蹤兩者是否再度同向變動 |
