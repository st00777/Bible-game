...
## 13. 待補強的設計問題（請 Claude Design 給建議）

> 以下是團隊內部已知的視覺痛點，希望在不改技術約束（純 CSS + emoji）的前提下提升設計感。

### 13.1 整體美術方向定位
**現況**：CSS 風格偏「療癒卡片風」，但缺少統一的視覺主題。CLAUDE.md 寫待開發功能：「介面美化（免費素材，可愛風，方向未定：像素 vs 插畫）」。
**問題**：
- 是否該強化某個方向？（例如：水彩風 / 童書插畫風 / 極簡禪意風 / 像素風）
- 若要堅持「純 emoji + CSS」，有哪些 CSS 技巧可以加強視覺差異化（如：手繪邊框、紙張紋理、SVG 雜訊濾鏡）？

### 13.2 Emoji 視覺一致性
**現況**：87 個章節裝備 emoji 來自不同 emoji 集，視覺風格不統一（有的扁平、有的立體、有的線條、有的填色）。
**問題**：是否有方法用 CSS filter（如 `grayscale + tint`）統一視覺風格？或建議規範哪些 emoji 應避免使用？

### 13.3 書架 vs 主頁色彩反差
**現況**：主頁是天空藍漸層 + 米白卡片，書架頁是深棕木紋。切換時視覺跳躍感強。
**問題**：書架要不要改設計？或主頁要不要加一塊木紋元素呼應？

### 13.4 成就解鎖儀式感
**現況**：bounce 動畫的金色 emoji + verse 卡。儀式感尚可但「平淡」。
**問題**：純 CSS 能怎麼放大「終於解鎖」的情緒高潮？（光暈、粒子、scale 序列、conic-gradient 旋轉光暈？）

### 13.5 長者友善的視覺優化
**現況**：已有三段字體切換，但對比度、按鈕命中區、icon 辨識度未專為長者調整。
**問題**：在不破壞療癒風的前提下，長者模式還可以加強什麼？

### 13.6 暗黑模式（未實作）
**現況**：完全沒有 dark mode。深夜 22-05 時段佔總使用 50%。
**問題**：是否該做暗黑模式？若要做，色彩語意如何遷移？

### 13.7 教會場域氛圍
**現況**：視覺風格中性偏可愛，沒有強烈宗教元素（沒用十字架做主視覺等）。
**問題**：要保持「中性溫暖」還是該加入更明顯的教會視覺語彙？這需要平衡「不嚇跑慕道友」與「教會內部認同感」。
