...
- [⏳] **曠野呼聲 v2 多輪對話**（進行中，2026-05-05 開始 Phase 2）
  - Phase 1 ✅ 完成（2026-05-05，commit ffa9545）── 資料層（rules + messages 子集合 + 15 筆 v1 文件 migrate）
  - Phase 2 ⏳ 進行中 ── 玩家端 UI（wantReply / 我的留言 / thread / 紅點 / toast）
  - Phase 3 ⏳ 待開始 ── 管理後台（獨立 Hosting site + Cloud Function 30 天 auto closed）
...
- **2026-05-08 開發協調**：曠野呼聲 v2 Phase 1-2B 實作中
  的設計決策（lazy-init B 方案、容錯規則三分流），
  屬「執行性決策」歸開發協調，無需經 PM。
- **2026-05-08 美術協調**：design-system.md 13.13 從
  「推薦施工順序（依 ROI）」改寫為「施工順序由 PM 視窗統籌」
  （commit 527d204）。施工順序屬「策略性決策」，
  美術協調主動把排程權交還 PM。
...
- PM 視窗使用 conversation_search 跨視窗讀取脈絡，
  整合判斷後給出明確歸屬
