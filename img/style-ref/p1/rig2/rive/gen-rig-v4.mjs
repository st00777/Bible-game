...
const PARTS = new URL("./parts-q/", import.meta.url).pathname;
...
groups.find((g) => g.id === "armSolo").active = "armBack";
{
  ...
  for (const [gid, z] of [["armBack", 61], ["armFront", 85]])
    images.push({ id: `img_armR_${gid}`, pngPath: `${PARTS}/armR.png`, x: l.x, y: l.y, parent: gid, z,
...
const lift = {
  ...
  tracks: [
    { target: "bone_uarm_R", property: "rotation", keyframes: K([[0, uR], [22, uR - 128, "ease-out-back"], [70, uR - 128, "hold"], [100, uR, "ease-in-out"]]) },
    { target: "bone_farm_R", property: "rotation", keyframes: K([[0, fR], [22, fR - 38, "ease-out"], [70, fR - 38, "hold"], [100, fR, "ease-in-out"]]) },
...
const out = process.argv[2] ?? "rig-v4.scene.json";
writeFileSync(out, JSON.stringify(scene, null, 1));
writeFileSync("rig-v4.args.json", JSON.stringify({ scene, outPath: `${process.cwd()}/traveler-v4.riv`, previewTime: 0 }));
