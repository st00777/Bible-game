...
// 畫序：披風後片 → 下身 → 上身 → 頭 → 眼皮 → 髮（試做版無手臂：James 2026-09-10 指出獨立手臂疊披風像破洞，改為手臂藏披風下；正式版手臂烤進下身圖）
...
const lidOpen = { name: "LidOpen", fps: 60, duration: 2, loop: "loop", tracks: [{ target: "img_sit_lid", property: "opacity", keyframes: K([[0, 0]]) }] };
...
const scene = {
  ...
  stateMachine: { name: "SM", inputs: [{ name: "blink", type: "trigger" }], layers: [
    ...
    { name: "Face", states: [{ name: "Open", animation: "LidOpen" }, { name: "Blink", animation: "Blink" }], transitions: [
      { from: "entry", to: "Open" }, { from: "Open", to: "Blink", condition: { input: "blink" }, durationMs: 50 }, { from: "Blink", to: "Open", exitTimeMs: 230, durationMs: 0 } ] },
  ] },
};
writeFileSync("sit-v1.scene.json", JSON.stringify(scene, null, 1));
writeFileSync("sit-v1.args.json", JSON.stringify({ scene, outPath: `${process.cwd()}/sit-v1.riv`, previewTime: 0 }));
