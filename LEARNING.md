...
2026-08-01 彼後 1:11 驗證時發現 hkbs 非 CUNP1 路徑回 HTTP 200 卻是和修版（靜默失敗）；現行規則（唯一來源、識別區塊查驗、錯誤路徑三條、判準）一律以 **CLAUDE.md「經文來源查驗標準（正本）」** 為準，此處不另存副本（2026-08-20 收斂）。

### 【八】長期 feature branch 與 cherry-pick 發布都是反模式
...
（2026-08-24 自 CLAUDE.md「分支策略 > 歷史補註」搬入）
