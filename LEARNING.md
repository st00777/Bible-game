...
## 2026-05-28 W22：dev↔main 27 commit 分岔的對齊治本

### 事件背景
W22 要把 B1（事件流）+ E1（個人資料）從 dev 上 main，盤點才發現 **dev 與 main 已嚴重分岔**：另一視窗把 v2.14 release（5 連發 commit + flag 翻轉）整批做在 **main** 上、沒反向 merge 回 dev。結果：
- main 領先 dev **27 commit**（含 v2.14 完整 release）
- dev 領先 main 3 commit（B1×2 + E1×1）
- 兩分支關鍵 flag **完全相反**：main `FEATURE_FEEDBACK_V2=true`/`SUPPRESS_VERSION_POPUP=false`，dev 反之

**若直接 `dev→main` ff/merge 會災難回退**：把 main 上已開放的曠野呼聲 v2 入口關掉、把版本公告靜音。

### ✅ 學到的：破壞性 merge 要有「五道安全網 + 反向工序」
1. **先 tag 錨點**：動手前 `git tag pre-align-v214-backup origin/main`，解錯能一鍵回滾。
2. **工序反向（關鍵）**：不在 main 賭一次解對，而是 **main merge 進 dev、在 dev/worktree 解衝突 + preview 驗證**，全綠才把對齊結果上 main。
3. **地雷 flag 人工 grep 確認、不信 auto-merge**：這次 ort 三向合併「零 conflict」自動完成——正因為 dev 從共同祖先後沒動過那兩個 flag、main 單邊改，git 判定無衝突直接採 main 值。**但「沒 conflict」不等於「值對」**，仍須 `grep` 把 flag 實際值貼出來確認（feedback=true/popup=false）才往下。
4. **固定測試版而非臨時 channel**：對齊結果部署到固定 site `bible-game-bcb84.web.app`（CORS 對 Cloud Functions 已加白、能跑 AI 默想），比臨時 channel 適合完整驗證。
5. **push main 留給人**：classifier 擋 default 分支 push、且玩家上線值得 James 親手按。

### ✅ 治本：對齊後兩分支同 HEAD
最終 `origin/main = origin/dev = 148aa94`（v2.15）。分岔消滅、未來 dev→main 又能簡單前進。

### ⚠️ 往後注意
- **每完成一個 release 就反向 merge 回 dev**，別讓 main 單邊累積——這次 27 commit 分岔的根因就是「release 只做在 main、忘了同步 dev」，是「教訓 1 長期 feature branch 反模式」的鏡像版（這次換成 main 單飛）。
- **多視窗共寫同 repo**：release 類動作要嘛集中一個視窗做完整（含反向 merge），要嘛明確分工，避免兩條線各自前進。

## 2026-06-05（W23）

### editDuration「覆蓋率低」≠ 半殘，要先分清「沒上線」vs「真有 bug」

**✅ 學到的**
- editDuration 覆蓋率只有 11%，第一直覺是「埋了但 guard 漏觸發、需補捉」——
  錯。真因是計時器程式碼只在 dev，main/prod 還停舊版，玩家跑舊碼當然永遠不帶。
- 決定性證據是 `git grep _reflEditStart main = 0`（prod 根本沒這段碼），
  加上唯一跑過新碼的測試帳號連 11 筆 100% 帶、無 0/NaN → guard 本來就好的。
- 11% 拆開看：69% 是上線前的舊默想（結構上不可能帶），其餘是 prod 沒碼。

**⚠️ 往後注意**
- 看到「某指標覆蓋率低」，先問一句「這段碼上 prod 了嗎？」再問「是不是 bug」。
- 用 `git grep <關鍵字> main` 對照 prod 實況，比憑前端邏輯猜更快更準。
- 別把「還沒上線」誤判成「半殘需補捉」而開一張不必要的修 code 工單。

### GA4 事件命名對齊：改「消費端」，不要改 emit 端

**✅ 學到的**
- GA4 漏斗用設計名（read_verse_view / reflection_submit）查，但實際 emit 是
  question_view / submit_reflection（命名顛倒），導致漏斗永遠顯示 0、誤導判讀。
- 修法選「改消費端」：讓查詢/分析端改用「實際 emit 名」去抓 → 零程式風險、
  不斷歷史。改 emit 端雖能讓名字一致，但會斷掉累積數月的 GA4/Firestore 歷史連續性。

**⚠️ 往後注意**
- 命名不一致時，預設改「讀的那端」（查詢/儀表板），不要動「寫的那端」（emit）。
- 「改 emit 端 vs 改消費端」是取捨、不是純執行——牽涉歷史連續性，要先停下確認。

### 部署順序紀律：先 deploy functions、後 push main（mood AI 零空窗）

**✅ 學到的**
- v2.16 含 mood-aware AI（前端傳 mood、aiReflection 用 mood）。
- 上線順序守對：先 `firebase deploy --only functions:aiReflection`、後 push main
  → 玩家拿到新前端時，後端已經能接 mood，沒有「新前端打到舊後端」的空窗。
- 反過來（先 push main 後 deploy functions）會有一段時間前端送 mood、後端不認，
  可能觸發 fallback 或錯誤。

**⚠️ 往後注意**
- 前後端有相依的 release，永遠「先部署被呼叫的一端（後端），再放出呼叫端（前端）」。
- deploy.sh 已封裝 functions / hosting / channel 三個短指令，避免 Agent View 長指令格式 bug。

## 2026-08-01 批次9A/9B 收網 — 選節、標點、來源驗證七條

### 【一】話語體書卷的三類選節陷阱
啟示錄等話語體書卷，選節時內容合適不等於可用。金句卡與裝備是單節獨立呈現，必須另驗單獨呈現時的標點完整性。三類結構性陷阱：
① 七封信「得勝應許」句永遠是書信引語收尾，帶孤立閉引號，換節無法迴避
② 長篇話語跨節延續（1:17→1:20、7:14→7:17、12:10→12:12），單節取出必然引號不閉合
③ 跨節括號插入語（16:14 開括號、16:15 閉括號），單節取出留下孤兒符號
9A 因此換 6 節，9B 因此換 5 節。（2026-08-01）

### 【二】小字註不是正文
和合本小字註（「或譯」「原文是」「有古卷是」）不是正文，導讀與選節一律剝除。
9B 證據：27 節中至少 4 節帶小字註（10:6 或譯：不再耽延了／10:11 原文是他們／12:16 原文是河／15:6 有古卷是寶石），全數剝除正確。10:11 為最佳例證——若採小字註，主詞會從「天使」變成「他們」，語意全變。
已納入 9C 規格：要求 CC 主動列出帶小字註的選節。（2026-08-01）

【補充：hkbs 頁面的三種標記，處理方式各不相同】

  <i>    專名斜體（人名、地名）
         → 屬正文，保留文字。斜體僅為排版標示。
         例：<i>亞當</i>、<i>該隱</i>、<i>伊甸園</i>

  <ins>  小字補字（原文無、中文語法需要補的字）
         → 屬正文，保留。這是譯文的一部分。
         例：創 2:7「他就成了有靈的活人，<ins>名叫</ins>亞當」
             ——剝除會變成「有靈的活人，亞當」，語意殘缺

  <sup>  譯註（或譯／原文是／有古卷是）
         → 須剝除。這才是教訓【二】所指的「小字註」。
         內容在 title 屬性裡，頁面上只顯示一個上標記號，
         容易漏看。
         例：創 4:1 <sup title="就是得的意思"></sup>
             徒 20:22 <sup title="原文是心被捆綁"></sup>

⚠️ 「小字註不是正文」這句話指的是 <sup> 譯註，
不是所有以小字排印的內容。<ins> 補字是小字但屬正文，
剝除會造成漏字——那是語意層錯誤，依分級規則須立即升級。

判準：看該標記的內容「拿掉之後句子是否仍完整」。
<ins> 拿掉會殘缺（屬正文）；<sup> 拿掉不影響（屬註釋）。
遇到本表未列的新標記，用此判準判斷，並回報內容視窗。

抽取器現行行為正確，維持不動。
此條為預防性紀錄，避免未來視窗誤改。
（2026-08-17，創世記 GEN1-5 生產時由內容視窗發現）

### 【三】忠實呈現的前提是自己先讀對原文
要求「忠實呈現」的前提是自己先讀對原文。下 outline 或規格定稿前，若涉及具體經文內容（人物、動作、後果），必須先核原文再寫，不可憑印象。
三次同型事件（2026-07-26 啟 2:22-23 主詞誤植並造成落檔錯誤；2026-08-01 規格誤寫啟 9:4 正反相反；同日誤判 9:13 句末標點），後兩次皆由 CC 掃描時發現並停下回報，未造成落檔錯誤。
矯正：9C 規格起，選節理由只寫節號與選節動機，不寫「那節在講什麼」。

### 【四】跨視窗期間的前置盤查須對帳 origin
跨視窗協作期間的前置盤查，須同時驗本地 HEAD 與 origin 對帳，不可只驗本地。發布工單的 Phase 0 亦須同時記錄 origin/main 與本地 main 並確認相等——git worktree add ... main 檢出的是本地分支，不是遠端。
事件一：9A 修正工單只驗本地 HEAD，CC 完成後 push 被拒（期間數據視窗推進 62a2eb6）。
事件二：9B 發布時本地 main 領先遠端兩筆（PM 開出的文件批工單已完成但停在未 push 狀態），撞錨點偏差。
兩次 CC 皆停下回報、未自行處置，正確。
衍生規則：PM 開出的工單若結尾為「停在 push 前」，必須追到 push 完成才算結案——未推的工單不是完成，是埋著的雷。（2026-08-01）

### 【五】承認解釋分歧本身就是誠實
遇到歷來有多種理解的段落（如啟 12 婦人象徵），導讀明寫「歷來有不同的理解」並列出主要幾種，不做裁定。這既符合「不刪減」（有呈現），也不越權替玩家決定。優於兩種錯誤做法：迴避不提（＝刪減）、或擇一當定論（＝越權）。（2026-08-01，9B 批次）

### 【六】破折號與同位語連接是兩種不同的標點
不可互相援引先例。
· 破折號：U+2014 成對（——），語意轉折或補充說明。全檔 1907 組，零例外，皆為正確用法。
· 同位語連接：U+FF0D 單一（－），連接頭銜與稱號，如「主－救主」「主神－全能者」。照 hkbs 原文。
兩次偏離皆源於把後者當成前者處理：批次7 PE2_1 bonusItem.desc、批次8 JUD1 baseItem.desc，兩處皆誤用單一 U+2014。批次8 時 PM 援引批次7 的破折號先例，未檢查該先例是否適用於此字元——而 PE2_1 本身就是第一個錯誤。

機檢規則（三條）：
① 內容欄中「單一 U+2014」= 0。正確破折號永遠成對，出現單一即為同位語被誤處理，必然是錯的。
② 非經文欄禁 U+FF0D；經文欄不檢查（照 hkbs 原文）。
③ U+2500 檢查排除程式註解區（452 處分隔線為正常裝飾，內容欄為 0）。

核心教訓：驗數量不等於驗型態。批次8 當時數了「破折號 count＝1」，數量對、型態沒驗，因此放行。

另註：「耶和華神」（創世記）不帶連接字元，直接相連（hkbs CUNP1 創 2 全章 11/11 一致，獨立驗證）。同位語連接號僅用於頭銜同位語結構，不可類推套用。（2026-08-01）

### 【七】驗證來源的 URL 必須指定到 CUNP1
hkbs 的 /CUNP/ 與 /CUNPSS/ 路徑皆回 HTTP 200，但內容為和合本2010（和修版 RCUV），非新標點和合本。唯 /CUNP1/ 為正確來源（title 實查「新標點和合本(神) | 和合本網路聖經」）。
此為靜默失敗：無 404、無錯誤訊息，抓到的是另一個譯本且看起來一切正常。差異可辨識，例：彼後 1:11 和修版作「永遠的國度」、新標點作「永遠的國」。
規則：所有經文核對一律使用 https://rcuv.hkbs.org.hk/CUNP1/{書卷}/{章}/ 並在抓取後查驗 title 確認版本，不可只看 HTTP 200。（2026-08-01，彼後 1:11 驗證時發現）
